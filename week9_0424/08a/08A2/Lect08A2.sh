#Bird2003_PB for Palu
#Original batch file is frpm Hsiu-Fang Lee, Department of Geosciences, NTU
#Edited by Bo_Han Tsao, Department of Geosciences, NTU for GMT6.0

prefix=Lect08A2
range=105/145/-15/10
grd=@earth_relief_15s.grd  
topo=cut.nc
...
gmt begin $prefix jpg A+m0.5c
    ...
    #======= Add legend ====================
	gmt legend -Dg145.0/-15.0+w4.5c+o0.0c/1.0c+jTR -F+p1p+gazure+s --FONT_ANNOT_PRIMARY=16p,7	
    #add GPS
    gmt plot GPS_AndreaWalpersdorf_Table1.txt -St0.2c -Gred -W0.5,0 -V
    gmt velo GPS_AndreaWalpersdorf_Table1.txt -Se0.015/0.95/0p -A0.15i+a30+e -W1.5,black -Gblack -V
    awk '{ print $1, $2, $8 }' GPS_AndreaWalpersdorf_Table1.txt | gmt text -F+f8p,0,blue -D0/-0.1 -V
    #The MORVEL angular velocity for AU relative to SU 
	echo 118.000 -12.000 16.5 70.5 0 0 0.7557 1  | gmt velo -Se0.015/0.95/0 -A0.3i+a30+e -W6p,red -Gred -V
    echo 118.000 -12.000 72.4 mm/yr | gmt text -F+f16p,7,red+jMC -D0.0/-0.1 -V
    #The MORVEL angular velocity for PS relative to SU 
    echo 131.000 2.500 -99.2 38.6 0 0 0.2300 1 | gmt velo -Se.015/0.65/0 -A0.3i+a30+e -W6p,red -Gred -V
    echo 131.000 2.500 106.4 mm/yr | gmt text -F+f16p,7,red+jMC -D0.0/-0.1 -V
    #add velo symbol
    echo 128.000 -18.000 40 0 2	2 0 1 | gmt velo -Se.015/0.95/0 -A0.15i+a30+e -W1.5,black -N -V
