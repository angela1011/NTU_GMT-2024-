# original batch file from Pei-Ling Wang and modified by Hsiu-Fang Lee, Department of Geosciences, NTU
# Edited by Bo-Han Tsao and Jyr-Ching Hu, Department of Geosciences, NTU for GMT 6.0
#
# set variables
prefix=Lect06A
range=119.2/122.8/21.4/25.5
grd=../grdfiles/tw80m.nc
cpt1=topo_gray.cpt
cpt2=fpcolor.cpt
# configure
...
gmt begin $prefix jpg A+m0.5c
    ...
    # ====== 1999 Sort by magnitude ===========
    awk '{ if ($4 < 2)  print $1, $2, $3}' 1900-201907EQs.txt | gmt plot -C$cpt2-Sc.01 -V
    awk '{ if ($4 >= 2 && $4 < 3) print $1, $2, $3}' 1900-201907EQs.txt | gmt plot -C$cpt2 -Sc.01 -V
    awk '{ if ($4 >= 3 && $4 < 4) print $1, $2, $3}' 1900-201907EQs.txt | gmt plot -C$cpt2 -Sc.03 -V
    awk '{ if ($4 >= 4 && $4 < 5) print $1, $2, $3}' 1900-201907EQs.txt | gmt plot -C$cpt2 -Sc.04 -V
    awk '{ if ($4 >= 5 && $4 < 6) print $1, $2, $3}' 1900-201907EQs.txt | gmt plot -C$cpt2 -Sc.06 -V
    awk '{ if ($4 >= 6 && $4 < 7) print $1, $2, $3}' 1900-201907EQs.txt | gmt plot -C$cpt2 -Sc.09 -V
    ...
    awk '{print $1, $2, $3*0.022}' epicenters_2010.txt | gmt plot -Sa -G0/0/0 -V
    gmt meca 201011-201904CMT.txt -Sa0.3i+f8p+o2p -A -V
    awk '{print $1, $2, $3*0.022}' epicenters_201011-201904.txt | gmt plot -Sa -G0/0/0 -V
